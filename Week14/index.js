...
let element = "ul";
//get items in favoritesList as the value. If no values, make empty list
let list = JSON.parse(localStorage.getItem('favoritesList')) || [];
...
function addToFavorites() {
  ...
  //create a message if there is no activity in output to add to favorites
  ...
  fetch(apiURL)
    ...
    .then((jsObject) => {
      ...
      if (jsObject.error == 'No activity found with the specified parameters') {
        alert("Error: " + jsObject.error + ". Please try again with different parameters or select Random Activity.");
